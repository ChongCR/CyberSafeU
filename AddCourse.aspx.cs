    ...
    public partial class AddCourse : System.Web.UI.Page
    {
        ...
        private void BindModuleList()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@CourseCode", ViewState["CourseCode"].ToString());
                    connection.Open();
                    DataTable dt = new DataTable();
                    using (SqlDataAdapter adapter = new SqlDataAdapter(command))
                    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (FileUpload1.HasFile)
            {
                ...
                ViewState["imgurUrl"] = imgurUrl;

           
                imagePreview.Src = imgurUrl;
            }
            ...
            else
            {
                if (!string.IsNullOrEmpty(existingCourseCode))
                {
                    ...
                    UpdateCourseData(existingCourseCode);
                    multiView.SetActiveView(view2);
                    BindModuleList();
                    BindAssessmentGridView();
                    PopulateModules(ViewState["CourseCode"].ToString());
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            else
                            {
                                ...
                                InsertCourseData();
                                multiView.SetActiveView(view2);
                                lblCourseTitle.Text = ViewState["courseTitle"].ToString();
                                Label3.Text = ViewState["CourseCode"].ToString();
        ...
        private void UpdateCourseData(string courseCode)
        {
            try
            {
                if (ViewState["imgurUrl"] != null)
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        using (SqlCommand command = new SqlCommand(updateQuery, connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@CourseCategory", ViewState["category"].ToString());
                            ...
                            command.Parameters.AddWithValue("@ImageURL", ViewState["imgurUrl"].ToString());
            ...
            {
                Console.WriteLine("Error updating course information: " + ex.Message);
        ...
        protected void PopulateInstructorDropdown()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, connection))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        instructorDropdown.Items.Add(new ListItem(instructorName, instructorID.ToString()));
                    }
