<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" validateRequest="false" CodeBehind="EditCourse.aspx.cs" Inherits="WebApplication1.EditCourse" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
            ...
            <div class="row">
                <div class="col-12 text-center">
                        <div class="form-group">
                         <img id="imagePreview" runat="server" src="" alt="Image Preview"
    style="max-width: 100%; max-height: 100%; width: auto; height: auto; display: block; margin: auto;" />
      ...
      <div class="container text-dark form-border">
            ...
            <div class="row">
                ...
                <div class="row">
                    <div class="col-12">
                        <asp:Button ID="Button5" runat="server" Text="Back" OnClick="btnBack_Click2" CssClass="btn btn-secondary float-left" />
                        <asp:Button ID="Button2" runat="server" Text="Update" OnClick="btnUpdate_Click2" CssClass="btn btn-primary float-right" />
                    </div>
                </div>
                                            ...
                                            <asp:Repeater ID="RepeaterAnswers" runat="server">
                                               <ItemTemplate>
                                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                                       ...
                                                       <span class="badge <%# Convert.IsDBNull(DataBinder.Eval(Container.DataItem, "IsCorrect")) ? "badge-secondary" : (Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "IsCorrect")) ? "badge-success" : "badge-danger") %>">
                                                            <%# Convert.IsDBNull(DataBinder.Eval(Container.DataItem, "IsCorrect")) ? "Unknown" : (Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "IsCorrect")) ? "Correct" : "Incorrect") %>
                                                        </span>
                                                    </div>
                                                </ItemTemplate>
                                            </asp:Repeater>
    ...
    <script>
        ...
        function toggleButtonColor2(button, answerId) {
            // Prevent the default action (postback)
            event.preventDefault();

            // Get the hidden field
            var hiddenField = document.getElementById('<%= HiddenFieldCorrectAnswer.ClientID %>');

            // Get current values and split into an array, removing empty strings
            var currentValues = hiddenField.value.split(';').filter(function (v) { return v !== ""; });

            // Check if the button is already marked as a correct answer
            var isCurrentlyCorrect = button.classList.contains('btn-success');

            // If the answer is currently marked as correct, remove it
            if (isCurrentlyCorrect) {
                button.classList.remove('btn-success');
                button.classList.add('btn-outline-secondary');
                // Remove the answer ID from the currentValues array
                currentValues = currentValues.filter(function (v) { return v !== answerId; });
            } else {
                // If the answer is not marked as correct, mark it as correct
                // First, clear all other buttons marked as correct if you want only one correct answer
                var answerButtons = document.getElementsByClassName('answer-button');
                for (var i = 0; i < answerButtons.length; i++) {
                    answerButtons[i].classList.remove('btn-success');
                    answerButtons[i].classList.add('btn-outline-secondary');
                }
                // Clear the currentValues since we are setting a new correct answer
                currentValues = [];

                button.classList.add('btn-success');
                button.classList.remove('btn-outline-secondary');
                // Add the answerId to the currentValues array
                currentValues.push(answerId);
            }

            // Update the hidden field value to the new set of correct answer IDs
        ...
        function previewImage2() {
            var preview1 = document.getElementById('<%= Img2.ClientID %>');
            var fileInput1 = document.getElementById('FileUpload1');
            var customFileLabel1 = fileInput1.nextElementSibling;
            ...
            if (file1) {
                if (file1.type.startsWith('image/')) {
                    ...
                    customFileLabel1.innerHTML = file1.name;
                } else {
                    alert('Please choose an image file.');
                    fileInput1.value = '';
                    customFileLabel1.innerHTML = "Choose Image";
        ...
        function previewImage() {
            ...
            var customFileLabel = fileInput.nextElementSibling;
            ...
            if (file) {
                if (file.type.startsWith('image/')) {
                    ...
                    alert('Please choose an image file.');
                    fileInput.value = '';
                    customFileLabel.innerHTML = "Choose Image";
        ...
        function previewFile(fileInput, previewElementId) {
            ...
            if (fileInput.files && fileInput.files[0]) {
                ...
                // If no file is selected, hide the preview element
        ...
        function displayImagePreview(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    var imgPreview = document.getElementById('<%= Img3.ClientID %>');
                    if (imgPreview) {
                        imgPreview.src = e.target.result;
                        imgPreview.style.display = 'block';
                    } else {
                        console.error("The image preview element was not found.");
                    }
                };
           ...
           // Get current values and split into an array
           var currentValues = hiddenField.value.split(';').filter(function (v) { return v !== ""; });

           // Toggle the button color and update the array
           if (button.classList.contains('btn-success')) {
               // Button is green, remove the answer ID from the array
               button.classList.remove('btn-success');
               button.classList.add('btn-outline-secondary');
               currentValues = currentValues.filter(function (v) { return v !== answerId; });
           } else {
               // Button is not green, add the answer ID to the array
               button.classList.add('btn-success');
               button.classList.remove('btn-outline-secondary');
        ...
        function SetImageUrl(imageUrl) {
            console.log('SetImageUrl function called with URL:', imageUrl);
